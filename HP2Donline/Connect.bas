...
Function getStartSN128(part_number As String, part_version As String) As String
On Error GoTo errorHandler
     Dim maxSerial As Integer
     Dim yearFlag As String
     
     yearFlag = Chr(Year(Now) - 2015 + 14 + 65)     ''2015 : O, 2016 : P, 2017 : Q...
    ...
    maxSerial = cmd("maxSerial")
    getStartSN128 = part_number + "" + part_version + yearFlag + getWeekOfYear(Now) + Right$("00" + dectohex(maxSerial), 2)
    conn.Close
    Set cmd.ActiveConnection = Nothing
    Exit Function
errorHandler:
   getStartSN128 = Err.Description
End Function

Function saveMaxSerial(startString As String, qty As Integer) As String
On Error GoTo errorHandler
    If conn.State = 0 Then
        conn.ConnectionString = Connect.getConnectionstring
        conn.Open
