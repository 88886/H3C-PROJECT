VERSION 5.00
Begin VB.Form frm21H3CPrint2DMAC 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "H3C整机模块类2DMAC标签逆向打印"
   ClientHeight    =   5220
   ClientLeft      =   48
   ClientTop       =   408
   ClientWidth     =   10668
   ...
   Begin VB.PictureBox picH3C 
      ...
      Height          =   1932
      Left            =   120
      Picture         =   "frm21H3CPrint2DMAC.frx":0000
      ScaleHeight     =   1908
      ScaleWidth      =   10428
      TabIndex        =   6
      Top             =   120
   ...
   End
End
Attribute VB_Name = "frm21H3CPrint2DMAC"
...
Private Sub cmdPrint_Click()
  ...
  If txtSN.Text = "" Then
   ...
   End If
    ...
    If Len(Trim(txtMAC.Text)) <> 12 Then
        MsgBox "MAC长度必须为12位!"
        txtMAC.SetFocus
        Exit Sub
    End If
    ...
    If Me.chkAutoTest.Value + Me.chkNonAutoTest.Value < 1 Then
        MsgBox "自动测试机柜未带出,不能打印，请联系ME!", vbInformation + vbOKOnly, "未选择自动测试机柜"
        txtSN.SetFocus
        Exit Sub
    End If
...
'   rightstr = Right(txtSN.Text, 6)
   ...
   End If
    ...
    Else
        myVars.Item("MAC").Value = UCase(mac)
    End If
   ...
   'myApp.Visible = True
   Call Connect.addPrintedLabelMAC(Trim(str), Me.Name, mac)
...
Private Sub OpenLppx()
   ...
   Set myDoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\逆向标签模板\" & "逆向整机SN标签40x15x15.lab")
...
Private Sub txtVer_KeyPress(KeyAscii As Integer)
  If (KeyAscii = 13) Then
        ...
        rec.Open sql99, conn, adOpenKeyset, adLockOptimistic
        If rec.EOF = False Then
            If rec.Fields(0) = "No" Then
                Me.chkNonAutoTest.Value = 1
                Me.chkAutoTest.Value = 0
            ElseIf rec.Fields(0) = "Yes" Then
