...
Dim rsDS2 As New ADODB.Recordset
Dim rsFTPC As New ADODB.Recordset
Dim rsFTPC1 As New ADODB.Recordset
Dim sql As String
Dim s1 As String
Dim s2 As String
...
Dim conFTPC1 As New ADODB.Connection

Private Sub chkNonRoHS_Click()
   If chkNonRoHS.Value = 1 Then
      MsgBox "确认使用非环保!", vbOKOnly, "提示"
      chkRoHS.Value = 0
   Else
      chkRoHS.Value = 1
   End If
...
'      conn1.Open
   End If
   If conFTPC1.State = 0 Then
      conFTPC1.ConnectionString = "Provider=SQLOLEDB.1; Data Source=10.11.1.130;Initial Catalog=afg_active_90; User ID=sa; PWD=Flash123"
      conFTPC1.Open
   End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    End If
     ...
     If conn1.State = 1 Then
      ...
      Set conn1 = Nothing
   End If
   If conFTPC1.State = 1 Then
        conFTPC1.Close
        Set conFTPC1 = Nothing
   End If
...
'        End If
        ...
        'sql = "select part_number , part_revision from unit with (nolock) where serial_number = '" & str & "' "
        ...
        rsFTPC1.Open sql, conFTPC1, adOpenKeyset
        If rsFTPC1.EOF = True Then
            rsFTPC1.Close
            conFTPC1.Close
            ...
            Exit Sub
        Else
            If Left(rsFTPC1.Fields(0), 4) <> "HUVC" Then
                rsFTPC1.Close
                conFTPC1.Close
                MsgBox "该条码对应的机种不是单板HUVC开头，请确认"
                txtArea.Text = ""
                txtVendorCode.Text = ""
                txtVenderName.Text = ""
                txtContpactNo.Text = ""
            ...
            txtModel.Text = rsFTPC1.Fields(0)
            If Trim(txtRev.Text) = "" Then
                txtRev.Text = rsFTPC1.Fields(1)
            ElseIf Trim(txtRev.Text) <> rsFTPC1.Fields(1) Then
                MsgBox "第一个单元版本" & txtRev.Text & "与当前版本" & rsFTPC1.Fields(1) & "不一致"
                txtArea.Text = ""
                txtVendorCode.Text = ""
                txtVenderName.Text = ""
                txtContpactNo.Text = ""
...
'                txtModel.Text = "HUVC" & UCase(Mid(str, 3, 8))
'                'txtModel.Text = UCase(Mid(str, 3, 8))
'                stringtmpmodel = UCase(Mid(str, 3, 8))
'            Else
'                txtModel.Text = "HUVC03" & UCase(Mid(str, 1, 6))
'                'txtModel.Text = "03" & UCase(Mid(str, 1, 6))
'                stringtmpmodel = "03" & UCase(Mid(str, 1, 6))
'            End If
'        Else
'            If Left(str, 2) = "21" Then
'                modelno = "HUVC" & UCase(Mid(str, 3, 8))
'            Else
'                modelno = "HUVC03" & UCase(Mid(str, 1, 6))
'            End If
'        End If
'
'
...
'        conn1.Open
        ...
        'update by allen.yan to handle the taskorder unit
        'get the max unit_key
        sql = "select top 1 a.op_name,b.status from tracked_object_history a with(NOLOCK) left outer join tracked_object_status b with(NOLOCK) on a.tobj_key=b.tobj_key where a.tobj_key in (select max(unit_key) from unit with(NOLOCK) where serial_number like '" & Trim(txtArea.Text) & "%') order by a.complete_time desc"
'
'        rsDS.Open sql, conn1, adOpenKeyset
'        If rsDS.EOF = True Then
'            rsDS.Close
            Dim conFTPC As New ADODB.Connection
        ...
        sql = ""
        sql = "select type from SingleUnit where sn='" & Mid(txtModel.Text, 5, 8) & "'"
...
Private Sub OpenLppx()
   ...
   Set mydoc = myApp.Documents.Open("\\10.11.1.25\Public\Manufacture\标签模板\" & "单板出货标签.lab")
