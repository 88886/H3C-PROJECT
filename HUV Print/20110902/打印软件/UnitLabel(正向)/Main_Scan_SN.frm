...
Private Sub txtArea_KeyPress(KeyAscii As Integer)
    If (KeyAscii = 13) Then
        ...
        sql = "select top 1 a.op_name,b.status from tracked_object_history a with(NOLOCK) left outer join tracked_object_status b with(NOLOCK) on a.tobj_key=b.tobj_key where a.tobj_key=(select unit_key from unit with(NOLOCK) where serial_number='" & Trim(txtArea.Text) & "') order by a.tobj_history_key desc"
