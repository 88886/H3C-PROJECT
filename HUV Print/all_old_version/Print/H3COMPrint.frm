...
'alan
   If (KeyAscii = 13) Then
      If Len(Trim(txtPart.Text)) <> 8 Then
         ...
         Exit Sub
      End If
      sql = "select * from H3C_3COM_New where pn='" & Trim(txtPart.Text) & "'"
      ...
      If rec.EOF = True Then
         ...
         Exit Sub
      Else
        txtSN.Text = Trim(rec.Fields(1))
        txtModel.Text = Trim(rec.Fields(2))
        txtQty2.Text = rec.Fields(3)
        isPrint = rec.Fields(4)
        'MsgBox (isPrint)
        rec.Close
