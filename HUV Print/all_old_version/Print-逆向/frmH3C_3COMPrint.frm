...
Private Sub cmdPrint_Click()
   ...
   End If
   OpenLppx
   myVars.Item("SN").Value = txtSN.Text
   myVars.Item("Item").Value = Mid(txtSN.Text, 3, 8)
   ...
   sql = "select ChangNAL from H3C_3COM where SN='" & txtSN.Text & "' and ValidTo<='" & Date & "'"
...
Private Sub txtSN_KeyPress(KeyAscii As Integer)
   If (KeyAscii = 13) Then
      ...
      End If
      rec.Close
      sql = "select * from H3C_3COM where SN='" & Mid(txtSN.Text, 3, 8) & "'"
      rec.Open sql, conn, adOpenKeyset, adLockOptimistic
      If rec.EOF = True Then
         MsgBox "此产品编码未进行设置!"
         txtVer.Text = ""
