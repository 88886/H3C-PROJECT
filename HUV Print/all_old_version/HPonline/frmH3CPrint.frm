...
Private Sub txtSN_KeyPress(KeyAscii As Integer)
  If (KeyAscii = 13) Then
     ...
     If HP_pack_label = True Then
      ...
      sql = "select top 1 * from vH3C_HP_test where serial_number='" & Trim(txtSN.Text) & "' order by last_modified_time DESC"
      checkhp.Open sql, conn, adOpenKeyset, adLockOptimistic
      If checkhp.EOF = True And HP_pack_label = True Then
        MsgBox ("没有对应的HP条码！")
        txtSN.Text = ""
        txtSN.SetFocus
        checkhp.Close
