...
Private Sub cmdConfirm_Click()
   ...
   If op = "Insert" Then
      ...
      sql = "Insert into SingleUnit(ID,SN,type) " & _
            "Values(" & getmaxID("SingleUnit") & ",'" & txtSN.Text & "','" & txtXH.Text & "')"
   ...
   ElseIf op = "Update" Then
      sql = "Update SingleUnit set type='" & txtXH.Text & "'" & _
...
Private Sub cmdQuery_Click()
   ...
   If rec.State = 1 Then
      rec.Close
   End If
   sql = "select * from SingleUnit Where 1=1"
   If txtSN.Text <> "" Then
...
Private Sub renovate()
   sql = "select * from SingleUnit order by ID,SN"
   If rec.State = 1 Then
      rec.Close
   End If
   rec.Open sql, conn, adOpenKeyset, adLockOptimistic
