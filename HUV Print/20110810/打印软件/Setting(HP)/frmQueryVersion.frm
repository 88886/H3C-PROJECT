...
Private Sub cmdUpdate_Click()
   ...
   Else
      ...
      End If
   End If
   If Trim(txtMName.Text) <> Mid(Trim(txtSN.Text), 3, 8) Then
      MsgBox "机种名称不能对应产品序号!"
      txtMName.SetFocus
      Exit Sub
   End If
   If Trim(txtVersion.Text) = "" Then
      ...
      Exit Sub
   End If
   If Trim(txtTestTime.Text) = "" Then
      MsgBox "测试时间不能为空!"
      txtTestTime.SetFocus
      Exit Sub
   End If
   sql = "Update Version set Ver='" & Trim(txtVersion.Text) & "',Operator='" & Trim(txtOperator.Text) & "',TestTime='" & Trim(txtTestTime.Text) & "' where SN='" & mfgVersion.TextMatrix(mfgVersion.RowSel, 2) & "' and Model='" & Trim(txtMName.Text) & "'"
   status = Connect.excuteUpdate(sql)
   If status <> "" Then
...
Private Sub txtSN_KeyPress(KeyAscii As Integer)
   If KeyAscii = 13 Then
      If Len(Trim(txtSN.Text)) < 10 Then
         ...
         Exit Sub
      Else
        txtMName.Text = Mid(Trim(txtSN.Text), 3, 8)
      End If
   End If
End Sub
