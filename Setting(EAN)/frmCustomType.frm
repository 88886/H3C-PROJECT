...
Private Sub chk1D_Click()
    If chk1D.Value = 1 Then
        chk2D.Value = 0
    End If
End Sub

Private Sub chk2D_Click()
    If chk2D.Value = 1 Then
        chk1D.Value = 0
    End If
End Sub
...
Private Sub cmdAdd_Click()
   ...
   If chk1D.Value = 0 And chk2D.Value = 0 Then
        MsgBox "条码类别不能为空!!", vbExclamation + vbOKOnly, "条码类别空"
        Exit Sub
   End If
      ...
      If rcd.Fields(0) > 0 Then
            sql = "update tblCustomType set Type='" & IIf(chkH3C.Value = 1, "H3C", "Non-H3C") & "',BarcodeType = '" & IIf(chk1D.Value = 1, "1D", "2D") & "' where PartNumber= '" & Trim(txtModel.Text) & "'"
      ...
      Else
            sql = "Insert into tblCustomType(PartNumber,Type,BarcodeType) " & _
            "Values(N'" & Replace(Trim(txtModel.Text), Chr(13) & Chr(10), "") & "','" & IIf(chkH3C.Value = 1, "H3C", "Non-H3C") & "','" & IIf(chk1D.Value = 1, "1D", "2D") & "')"
            status = Connect.excuteUpdate(sql)
            If status <> "" Then
                MsgBox "新增资料失败!" & "原因是" & status, vbOKOnly + vbInformation, "新增失败"
            End If
