...
'On Error Resume Next
  ...
  If DateDiff("d", CDate(Trim(Me.lblMSday.Text)), Date) >= 0 Then
   ...
   If Me.chkFCC.Value + Me.chkNonFCC.Value = 0 Or Me.chkFCC.Value + Me.chkNonFCC.Value = 2 Then
      MsgBox "FCC认证没有设定或者设定不正确,不能打印!", vbInformation + vbOKOnly, "FCC认证不能为空"
      txtSN.SetFocus
      Exit Sub
   End If
   ...
   For i = 0 To qty - 1
      ...
      str = leftstr & Right("000000" & CLng(CLng(rightstr) + i), 6)
...
Private Sub txtSN_KeyPress(KeyAscii As Integer)
   If (KeyAscii = 13) Then
      ...
      sql = "select * from tblHUV where SN='" & Mid(Trim(txtSN.Text), 3, 8) & "' and HV='" & UCase(txtHV.Text) & "'"
      rec.Open sql, conn, adOpenKeyset, adLockOptimistic
      If rec.EOF = True Then
         MsgBox "此产品编码未进行设置!"
         txtVer.Text = ""
...
'        End If
        ...
        If IsNull(rec.Fields("FCC")) Then
            MsgBox "没有设定FCC,不能打印!", vbInformation + vbOKOnly, "FCC区域不能为空"
            txtSN.SetFocus
            Exit Sub
...
''        Set mydoc = myApp.Documents.Open("\\Sz-fs01\Public\Manufacture\标签模板\宇视发货标签NEW\" & "Uniview-NEW.Lab")
''   ElseIf Me.txtSalesArea.Text = "海外" Then
''        Set mydoc = myApp.Documents.Open("\\Sz-fs01\Public\Manufacture\标签模板\宇视发货标签NEW\" & "Uniview-海外.lab")
''   ElseIf Me.txtSalesArea.Text = "上海天跃" Then
''        Set mydoc = myApp.Documents.Open("\\Sz-fs01\Public\Manufacture\标签模板\宇视发货标签NEW\" & "Uniview-上海天跃.lab")
''   Else
''      MsgBox "没有销售区域,不能打印!", vbInformation + vbOKOnly, "销售区域不能为空"
''      Exit Sub
''      Unload Me
...
''   Set myObjs = mydoc.DocObjects
   ...
   If Me.txtSalesArea.Text = "中英文" Then
        Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\宇视发货标签NEW\宇视发货标签20140606\" & "Uniview-中英文.lab")
