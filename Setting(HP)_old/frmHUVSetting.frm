...
Private Sub cmdConfirm_Click()
   ...
   Dim CE, WEEE, ChinaRoHS, RoHS, TurkeyRoHS, SVPrint, CN3C, FCC, SalesLocation, CNCN, CNCA, ENCN, ENCA As String
   If chkCE.Value = 1 Then
      CE = "CE"
...
'可选项：浙江宇视科技有限公司、上海天跃科技股份有限公司、无（选无时不打印此变量）
  ...
  txtGW.Text = LCase(Trim(txtGW.Text))
   ...
   If op = "Insert" Then
      ...
      sql = "Insert into tblHUV(ID, HV, SN, CPN, EPN, Des, OS, GW, CE, WEEE, ChinaRoHS, RoHS, TurkeyRohs, MS, MSValidFrom, NAL, ValidFrom, PrintSV, CCC, SalesLocation, Remark, FCC, CNCN, CNCA, ENCN, ENCA, classification, model, rated_value, certification) " & _
            ...
            "'" & txtMS.Text & "','" & dtpMSValidFrom.Value & "','" & txtNAL.Text & "','" & dtpValidFrom.Value & "','" & SVPrint & "','" & CN3C & "','" & SalesLocation & "','" & txtRemark.Text & "','" & FCC & "','" & CNCN & "','" & CNCA & "','" & ENCN & "','" & ENCA & "', '" & classification & "', '" & model & "', '" & ratedValue & "', '" & certification & "' )"
   ...
   ElseIf op = "Update" Then
      sql = "Update tblHUV set CPN='" & Trim(txtCPN.Text) & "',EPN='" & Trim(txtEPN.Text) & "',Des='" & Trim(txtDes.Text) & "',OS='" & Trim(txtOS.Text) & "',GW='" & Trim(txtGW.Text) & "',CE='" & CE & "',WEEE='" & WEEE & "',ChinaRoHS='" & ChinaRoHS & "',RoHS='" & RoHS & "',TurkeyRohs='" & TurkeyRoHS & "'," & _
            ...
            "CNCN = '" & CNCN & "',CNCA = '" & CNCA & "',ENCN = '" & ENCN & "',ENCA = '" & ENCA & "',classification='" & classification & "', model='" & model & "', rated_value='" & ratedValue & "', certification='" & certification & "' " & " where ID=" & mfgH3C.TextMatrix(mfgH3C.RowSel, 1) & " and SN='" & Trim(txtSN.Text) & "'"
      status = Connect.excuteUpdate(sql)
      If status <> "" Then
...
Private Sub cmdDelete_Click()
   ...
   sql = "delete from tblHUV where ID=" & mfgH3C.TextMatrix(mfgH3C.RowSel, 1) & " and SN='" & mfgH3C.TextMatrix(mfgH3C.RowSel, 3) & "'"
   status = Connect.excuteUpdate(sql)
   If status <> "" Then
