...
''   if right(txt
   ...
   sql = "select * from revset  where model='" & txtsn.Text & "' and firstall='" & txtModel.Text & "' and endall='" & txtpass.Text & "' and ver='" & txtVer.Text & "'"
    ...
    If Asc(Mid(Right(txtModel.Text, 6), 1, 1)) < 48 Or Asc(Mid(Right(txtModel.Text, 6), 1, 1)) > 57 Then
        sql = "insert into revset(model,firstno,endno,ver,firstall,endall,normal,prefix) values('" & txtsn.Text & "','" & Right(txtModel.Text, 5) & "','" & Right(txtpass.Text, 5) & "','" & txtVer.Text & "','" & txtModel.Text & "','" & txtpass.Text & "','" & Mid(txtpass.Text, 12, 3) & "','" & Mid(Right(txtModel, 6), 1, 1) & "')"
    Else
