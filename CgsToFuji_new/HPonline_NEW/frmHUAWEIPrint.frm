...
'            str = "select top 1 part_number,part_revision,creation_time from unit wint(nolock) where serial_number='" & Trim(txtSN.Text) & "'"
      ...
      '============add by ben start=============
        ...
        Else
            ...
            "select W.order_number + '_' + U.part_number from [UNIT] as U left join [WORK_ORDER] as W " & _
            "on U.order_key = W.order_key where U.serial_number = '" & serial_number & "')  or A.bom_name in (select '_DEL_' + W.order_number + '_' + U.part_number from [UNIT] as U " & _
            "left join [WORK_ORDER] as W on U.order_key = W.order_key  where U.serial_number = '" & serial_number & "') "
            If rs5.State = 1 Then rs5.Close
            rs5.Open stringSQL
            If rs5.EOF = False Then
                flagHasBOM = True
    ...
    '=====================================================================
      ...
      Else
        ...
        Else
          ...
          rs9.Open sql, conn, adOpenKeyset, adLockReadOnly
