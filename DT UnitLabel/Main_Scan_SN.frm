...
Private Sub Form_Load()
   ...
   If conn1.State = 0 Then
      conn1.ConnectionString = "Provider=SQLOLEDB;User ID=datasweep;PWD=datasweep;Initial Catalog=dsActive;Data Source=DS-bak"
      conn1.Open
   End If
...
Private Sub txtArea_KeyPress(KeyAscii As Integer)
    If (KeyAscii = 13) Then
        ...
        sql = "select top 1 a.op_name,b.status from tracked_object_history a with(NOLOCK) left outer join tracked_object_status b with(NOLOCK) on a.tobj_key=b.tobj_key where a.tobj_key=(select unit_key from unit with(NOLOCK) where serial_number='" & Trim(txtArea.Text) & "') order by a.complete_time desc"
        ...
        sql = ""
        sql = "select type from SingleUnit where sn='" & stringtmpmodel & "'"
        ...
        If lstScanTmp.ListCount = 8 And CInt(Me.txtCodeNo.Text) = 8 Then
...
Private Sub OpenLppx()
   ...
   Set mydoc = myApp.Documents.Open("\\sz-FS01\Public\Manufacture\标签模板\BU1所有小项目标签模板\BU1 外箱label\大唐项目\" & "大唐单板出货标签.Lab")
