VERSION 5.00
Begin VB.Form frmSRIEPrint 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "SRIE Label Print"
   ClientHeight    =   12195
   ClientLeft      =   45
   ClientTop       =   405
   ClientWidth     =   16605
   ...
   Begin VB.PictureBox Picture1 
      Height          =   4695
      Left            =   130
      Picture         =   "frmSRIEPrint.frx":13652
      ScaleHeight     =   4635
      ScaleWidth      =   12195
      TabIndex        =   105
      Top             =   360
   ...
   Begin VB.Frame fmVar 
      ...
      Begin VB.PictureBox Picture13 
         Height          =   495
         Left            =   5520
         Picture         =   "frmSRIEPrint.frx":CC644
         ScaleHeight     =   435
         ScaleWidth      =   435
         TabIndex        =   97
         Top             =   5040
   ...
   End
End
Attribute VB_Name = "frmSRIEPrint"
...
'
      ...
      sql = "select case when Print_SV = 1 then 'Y' else 'N' end from tblSRIE where  Part_Number ='" & Mid(txtSN.Text, 3, 8) & "'  and Part_Revision = '" & txtHV.Text & "'"
      ...
      Else
            ...
            Else
                ...
                Else
                   ...
                   'add by robin for h3c temp print without test
                   
                           
                       Dim con9 As ADODB.Connection
                       Dim rs9 As ADODB.Recordset
        
                        Set con9 = New ADODB.Connection
                        Set rs9 = New ADODB.Recordset
                        con9.ConnectionString = "Provider=SQLOLEDB;User ID=sa;PWD=Flash123;Initial Catalog=afg_active_90;Data Source=10.11.1.130"
                        con9.ConnectionTimeout = 50
                        con9.Open
                        Set rs9.ActiveConnection = con9
                        rs9.CursorType = adOpenForwardOnly
                    
                        stringSQL = "select * from at_afg_h3c_labelPrint where valid_I =1 and partNumber_S = '" & Mid(txtSN.Text, 3, 8) & "' "
                        If rs9.State = 1 Then rs9.Close
                        rs9.Open stringSQL
                         If rs9.EOF = False Then
                        sql = "Select top 1  barcode, testtime, remark  FROM test_equ where barcode='" & Trim(txtSN.Text) & "' AND (rtrim(remark) <> '' and left(equipment,3) <> 'MTP' and remark is not null ) ORDER BY testtime DESC "
                         Else
    ...
    '=====================================================================
      ...
      '===========================================================
       ...
       sql = "SELECT [ID],[Part_Number],[Part_Revision],[ProductID],[CPN],[EPN],[Des],[Size],[GW],[MS],[NAL1],[NAL1_Title],[NAL2],[NAL2_Title]" & _
        ...
        ",[5000_State],[Remark],case when EAC is null then 'N/A' when EAC = 0 then 'No' when EAC = 1 then 'Yes' end as 'EAC' FROM [Print].[dbo].[tblSRIE] where Part_Number = '" & _
        Mid(txtSN.Text, 3, 8) & "' and Part_Revision ='" & txtHV.Text & "'"
      
      
      'sql = "select ID, HV, SN, CPN, EPN, Des, OS, GW, CE, WEEE, ChinaRoHS, RoHS, TurkeyRohs, MS, MSValidFrom, NAL, ValidFrom, PrintSV, Remark from tblSRIE where SN='" & Mid(txtSN.Text, 3, 8) & "' and HV='" & txtHV.Text & "'"
'       sql = "SELECT [ID],[Part_Number],[Part_Revision],[ProductID],[CPN],[EPN],[Des],[Size],[GW],[MS],[NAL1],[NAL1_Title],[NAL2],[NAL2_Title]" & _
'        ",[CE], WEEE,ChinaRoHS,[RoHS],[TurkeyRoHS],Ukraine,ATick,[ATick_ID],CTick,[CTick_ID],ICT,[ICT_ID],RCM,[RCM_ID],Gost,[Gost_ID],KC,[KC_ID],Print_SV,Print_Power,[5000_State],[Remark] FROM [Print].[dbo].[tblSRIE] where Part_Number = '" & _
'         Mid(txtSN.Text, 3, 8) & "' and Part_Revision ='" & txtHV.Text & "'"
...
Private Sub OpenLppx()
   ...
   ' sql99 = "select top 1 1 from tblSRIE  where FenXiao=1 and  Part_Number ='" & Mid(txtSN.Text, 3, 8) & "'  and Part_Revision = '" & txtHV.Text & "'"
   ' sql99 = "select top 1 isnull(FenXiao,0),isnull(Server,0) from tblSRIE  where Part_Number ='" & Mid(txtSN.Text, 3, 8) & "'  and Part_Revision = '" & txtHV.Text & "'"
   ...
   '  rec1.Close
    ...
    Set myDoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\SRIE模板\" & "SRIE发货模板.lab")
