...
'   End If
   If Trim(txtMaterial.Text) = "" Then
      MsgBox "物料代码不能为空!!", vbExclamation + vbOKOnly, "物料代码空"
      txtMaterial.SetFocus
      Exit Sub
   End If
'   If Trim(txtOrder.Text) = "" Then
'      MsgBox "销售订单不能为空!!", vbExclamation + vbOKOnly, "销售订单空"
'      txtOrder.SetFocus
'      Exit Sub
'   End If
'   If txtCPN.Text = "" Then
...
Private Sub cmdDelete_Click()
   ...
   status = Connect.excuteUpdate(sql)
   If status <> "" Then
      MsgBox "删除海康设定资料失败!" & "原因是" & status, vbExclamation + vbOKOnly, "删除失败"
   End If
      MsgBox "删除海康设定资料成功!", vbInformation + vbOKOnly, "删除成功"
   renovate ("")
End Sub
