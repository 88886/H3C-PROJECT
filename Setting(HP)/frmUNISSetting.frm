...
Begin VB.Form frmUNISSetting 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "UNIS Setting(Test)"
   ClientHeight    =   10545
   ClientLeft      =   45
   ClientTop       =   405
   ClientWidth     =   12120
   ...
   End
End
Attribute VB_Name = "frmUNISSetting"
...
'      CE = "CE"
  ...
  txtGW.Text = LCase(Trim(txtGW.Text))
   ...
   If op = "Insert" Then
      ...
      sql = "select Count(*) from tblUNIS where SN='" & Trim(txtSN.Text) & "' and HV='" & Trim(txtHV.Text) & "' "
      ...
      sql = "Insert into tblUNIS(ID, HV, SN, CPN, EPN, Des, OS, GW, CE, WEEE, ChinaRoHS, RoHS, TurkeyRohs, MS, MSValidFrom, NAL, ValidFrom, PrintSV, Remark) " & _
            "Values(" & getmaxID("tblUNIS") & ",'" & Trim(txtHV.Text) & "','" & Trim(txtSN.Text) & "','" & Trim(txtCPN.Text) & "','" & Trim(txtEPN.Text) & "','" & Trim(txtDes.Text) & "','" & Trim(txtOS.Text) & "','" & Trim(txtGW.Text) & "','" & CE & "','" & WEEE & "','" & ChinaRoHS & "','" & RoHS & "','" & TurkeyRoHS & "'," & _
   ...
   ElseIf op = "Update" Then
      sql = "Update tblUNIS set CPN='" & Trim(txtCPN.Text) & "',EPN='" & Trim(txtEPN.Text) & "',Des='" & Trim(txtDes.Text) & "',OS='" & Trim(txtOS.Text) & "',GW='" & Trim(txtGW.Text) & "',CE='" & CE & "',WEEE='" & WEEE & "',ChinaRoHS='" & ChinaRoHS & "',RoHS='" & RoHS & "',TurkeyRohs='" & TurkeyRoHS & "'," & _
...
Private Sub cmdDelete_Click()
   If mfgH3C.RowSel <= 0 Then
      ...
      Exit Sub
   End If
   sql = "delete from tblUNIS where ID=" & mfgH3C.TextMatrix(mfgH3C.RowSel, 1) & " and SN='" & mfgH3C.TextMatrix(mfgH3C.RowSel, 3) & "'"
   status = Connect.excuteUpdate(sql)
   If status <> "" Then
        ...
        Else
          ...
          '======================================================
           If K = 3 Then
             ...
             Dim rcd As New ADODB.Recordset
             sql = "select Count(*) from tblUNIS where SN='" & cellValue & "' and HV='" & cellhvValue & "'"
             rcd.Open sql, conn, adOpenKeyset, adLockOptimistic
             If rcd.Fields(0) > 0 Then
                If action = 0 Then
            ...
            If K = 19 Then
               If action = vbRetry Then
                   sql = "Update tblUNIS set CPN='" & xlSheet.Cells(j, 4) & "',EPN='" & xlSheet.Cells(j, 5) & "',Des='" & xlSheet.Cells(j, 6) & "',OS='" & xlSheet.Cells(j, 7) & "',GW='" & xlSheet.Cells(j, 8) & "',CE='" & xlSheet.Cells(j, 9) & "',WEEE='" & xlSheet.Cells(j, 10) & "',ChinaRoHS='" & xlSheet.Cells(j, 11) & "'," & _
...
'                   MsgBox "修改H3C设定资料成功!"
               ElseIf isexist = False Then
                   sql = "Insert into tblUNIS(ID, HV, SN, CPN, EPN, Des, OS, GW, CE, WEEE, ChinaRoHS, RoHS, TurkeyRohs, MS, MSValidFrom, NAL, ValidFrom, PrintSV, Remark) " & _
                        " Values(" & getmaxID("tblUNIS") & ",'" & xlSheet.Cells(j, 2) & "','" & xlSheet.Cells(j, 3) & "','" & xlSheet.Cells(j, 4) & "','" & xlSheet.Cells(j, 5) & "','" & xlSheet.Cells(j, 6) & "','" & xlSheet.Cells(j, 7) & "','" & xlSheet.Cells(j, 8) & "','" & xlSheet.Cells(j, 9) & "','" & xlSheet.Cells(j, 10) & "','" & xlSheet.Cells(j, 11) & "'," & _
...
Private Sub cmdQuery_Click()
    ...
    If rec.State = 1 Then
        rec.Close
     End If
     sql = "select * from tblUNIS Where 1=1"
     If txtSN.Text <> "" Then
...
Private Sub renovate()
   sql = "select * from tblUNIS order by ID,SN"
   If rec.State = 1 Then
      rec.Close
   End If
   rec.Open sql, conn, adOpenKeyset, adLockOptimistic
