...
On Error GoTo errorHandler
    ...
    maxSerial = cmd("maxSerial")
    getNewStartSN = part_number + part_version + Format$(Now, "yyMMdd") + Right$("000" + CStr(maxSerial), 4)
    conn.Close
    Set cmd.ActiveConnection = Nothing
    Exit Function
errorHandler:
...
On Error GoTo errorHandler
    ...
    maxSerial = cmd("maxSerial")
    getNewStartSN_6 = part_number + part_version + Format$(Now, "yyMMdd") + Right$("00" + CStr(maxSerial), 3)
    conn.Close
    Set cmd.ActiveConnection = Nothing
    Exit Function
errorHandler:
