...
Private Sub chkEP_Click()
    If MsgBox("非环保属性变更不能更改,确认请点击Yes,否则请点击No", vbYesNoCancel, "操作提醒") = vbYes Then
...
'      Exit Sub
   ...
   End If
    ...
    If UploadH3CInfo(Pb, Trim(txtSN.Text), Trim(txtVer.Text), Trim(txt5000.Text), Trim(txtPC.Text), "CHINA", "NXFHBeiJianPrint") = False Then
        MsgBox "资料保存失败不能打印!", vbInformation + vbOKOnly, "资料保存失败"
        txtSN.SetFocus
        Exit Sub
    End If
    
    If UploadH3C_PB(Pb, Trim(txtSN.Text), Trim(txtVer.Text), Trim(txt5000.Text), Trim(txtPC.Text), "CHINA", "NXFHBeiJianPrint") = False Then
...
'   End If
   ...
   If Me.txtHV.Text <> "/" And Me.txtHV.Text <> "N/A" Then
...
''        myVars.Item("Code").Value =
   ...
   Else
      If Trim(Me.txtUkraineID.Text) <> "" Then
...
'      myObjs("Software").Top = 10000
'      myVars.Item("Software").Value = "N/A"
'   Else
...
'        Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\" & "H3CNEW-新备件.lab")
'   Else
'        Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\" & "分销H3CNEW-新备件.lab")
'   End If

    OldFlag = True
    
   If rec1.EOF = False Then
        If OldFlag = True Then '使用旧模板
            ...
            ElseIf rec1.Fields(0) = False And rec1.Fields(1) = True Then
                Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\" & "服务器H3CNEW-新备件.lab")
            Else 'never process
        ...
        Else '使用新模板
            If rec1.Fields(0) = True And rec1.Fields(1) = True Then
                ...
                Set mydoc = myApp.Documents.Open("xx.lab")
            ElseIf rec1.Fields(0) = False And rec1.Fields(1) = False Then
                Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\新H3C地址模板\" & "H3CNEW-新备件.lab")
            ElseIf rec1.Fields(0) = True And rec1.Fields(1) = False Then
                Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\新H3C地址模板\" & "分销H3CNEW-新备件.lab")
            ElseIf rec1.Fields(0) = False And rec1.Fields(1) = True Then
                Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\新H3C地址模板\" & "服务器H3CNEW-新备件.lab")
            Else 'never process
                MsgBox "分销产品和服务器产品维护错误，不允许打印，请联系ME"
                Set mydoc = myApp.Documents.Open("xx.lab")
            End If
