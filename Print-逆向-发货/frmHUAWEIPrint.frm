...
'      Exit Sub
  ...
  If DateDiff("d", CDate(Trim(Me.lblMSday.Text)), Date) >= 0 Then
   ...
   End If
    ...
    If UploadH3CInfo(Pb, Trim(txtSN.Text), Trim(txtVer.Text), "NA", "N/A", "CHINA", "NXFH_HWBJPrint") = False Then
        MsgBox "资料保存失败不能打印!", vbInformation + vbOKOnly, "资料保存失败"
        txtSN.SetFocus
        Exit Sub
    End If
    
    If UploadH3C_PB(Pb, Trim(txtSN.Text), Trim(txtVer.Text), "NA", "N/A", "CHINA", "NXFH_HWBJPrint") = False Then
...
Private Sub OpenLppx()
   ...
   Set mydoc = myApp.Documents.Open("\\Sz-fs01\Public\Manufacture\逆向标签模板\逆向20120615\" & "HUAWEI-新逆向备件.lab")
