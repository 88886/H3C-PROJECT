VERSION 5.00
Begin VB.Form frmNewH3C_LiangPin 
   BackColor       =   &H00FFFFFF&
   ...
   Begin VB.Frame fmVar 
      ...
      Begin VB.PictureBox Picture13 
         Height          =   495
         Left            =   10200
         Picture         =   "frmNewH3C_LiangPin.frx":0000
         ScaleHeight     =   435
         ScaleWidth      =   435
         TabIndex        =   104
         Top             =   5160
   ...
   Begin VB.PictureBox picH3C 
      ...
      Height          =   4575
      Left            =   0
      Picture         =   "frmNewH3C_LiangPin.frx":2083
      ScaleHeight     =   4545
      ScaleWidth      =   13905
      TabIndex        =   3
      Top             =   480
   ...
   End
End
Attribute VB_Name = "frmNewH3C_LiangPin"
...
'      Exit Sub
   ...
   End If
    ...
    If UploadH3CInfo(PB, Trim(txtSN.Text), Trim(txtVer.Text), Trim(txt5000.Text), Trim(txtPC.Text), "CHINA", "NXFHLiangPinPrint") = False Then
        MsgBox "资料保存失败不能打印!", vbInformation + vbOKOnly, "资料保存失败"
        txtSN.SetFocus
        Exit Sub
    End If
    
    If UploadH3C_PB(PB, Trim(txtSN.Text), Trim(txtVer.Text), Trim(txt5000.Text), Trim(txtPC.Text), "CHINA", "NXFHLiangPinPrint") = False Then
...
'   End If
   ...
   'For i = 0 To qty - 1
      ...
      'updated by allen.yan for the cint exceed the upper limitation
...
''        myVars.Item("Code").Value =
   ...
   Else
      myVars.Item("Ukraine ID").Value = Trim(Me.txtUkraineID.Text)
   End If
...
Private Sub txtSN_KeyPress(KeyAscii As Integer)
   If (KeyAscii = 13) Then
      ...
      rec.Close
        ...
        ",[5000_State],[Remark],case when EAC is null then 'N/A' when EAC = 0 then 'No' when EAC = 1 then 'Yes' end as 'EAC' FROM [Print].[dbo].[tblH3CNew] " & _
...
'        Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\" & "H3CNEW-新备件良品.lab")
'   Else
'        Set mydoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\" & "分销H3CNEW-新备件良品.lab")
'   End If

OldFlag = True

   If rec1.EOF = False Then
        If OldFlag = True Then '旧模板
            ...
            ElseIf rec1.Fields(0) = False And rec1.Fields(1) = True Then
                Set myDoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\" & "服务器H3CNEW-新备件良品.lab")
            Else 'never process
        ...
        Else '新模板
             If rec1.Fields(0) = True And rec1.Fields(1) = True Then
                ...
                Set myDoc = myApp.Documents.Open("xx.lab")
            ElseIf rec1.Fields(0) = False And rec1.Fields(1) = False Then
                Set myDoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\新H3C地址模板\" & "H3CNEW-新备件良品.lab")
            ElseIf rec1.Fields(0) = True And rec1.Fields(1) = False Then
                Set myDoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\新H3C地址模板\" & "分销H3CNEW-新备件良品.lab")
            ElseIf rec1.Fields(0) = False And rec1.Fields(1) = True Then
                Set myDoc = myApp.Documents.Open("\\sz-fs01\Public\Manufacture\标签模板\H3C发货标签NEW\逆向H3CNEW\新H3C地址模板\" & "服务器H3CNEW-新备件良品.lab")
            Else 'never process
                MsgBox "分销产品和服务器产品维护错误，不允许打印，请联系ME"
                Set myDoc = myApp.Documents.Open("xx.lab")
            End If
